...
export default function EcommerceResponsivePage({ products }: { products: Product[] }) {
        ...
        <div className="fixed inset-0 z-50 flex md:hidden">
          ...
          <div className="relative w-72 max-w-sm bg-white h-full p-5 shadow-xl flex flex-col gap-4 animate-in slide-in-from-left duration-200">
            ...
            <nav className="flex flex-col gap-2">
              {['Furniture', 'Lighting', 'Decor', 'Office', 'Electronics', 'Kitchen', 'Fashion', 'Sports', 'Home'].map((item, idx) => (
                <a key={idx} href={`/itemlist?q=${item}`} className="py-2 px-3 rounded-md hover:bg-gray-50 text-sm font-medium text-gray-700">
                  {item}
                </a>
              ))}
            </nav>
